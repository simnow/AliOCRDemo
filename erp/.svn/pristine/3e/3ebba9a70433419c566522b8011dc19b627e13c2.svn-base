...
public class SealAtivitiApplyController  {
	...
	*/ 
	...
	public Message  getSaPageList(@RequestParam(value="state",required=true) int state
		...
		message.setSuccess(false);
		try {
			PageInfo<SealActivitiApply> sealActivitiApplyList =  saService.getSealApplyList(state,pagesize,pageno,uid);
	...
	*/ 
	...
	@ResponseBody
	public  Message  deleteSealApply(@PathVariable("id") int id){
		
		Message message = new Message();
		try {
			saService.deleteSealApply(id);		
		} catch (Exception ex) {
			message.setSuccess(false);
			message.setMsg(ex.getMessage());
		}
