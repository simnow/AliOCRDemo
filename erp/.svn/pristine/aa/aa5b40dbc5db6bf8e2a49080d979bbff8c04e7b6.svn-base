...
public class CardActivitiApplyController{
	...
	*/ 
	...
	@ResponseBody
	public  Message  deleteCardApply(@PathVariable("id") int id){
		Message message = new Message();
		message.setSuccess(false);
		try {
	...
	*/ 
	...
	@ResponseBody
	@RequiresPermissions(Contants.CARD_SELECT_PERMISSION)
	public  Message  getAcApplyPageList(@RequestParam(value="state",required=true) int state
		...
		message.setSuccess(false);
		try {
			PageInfo<CardActivitiApplyDto> cardActivitiApplyList = caService.getAcApplyPageList(state,pagesize,pageno,uid);
