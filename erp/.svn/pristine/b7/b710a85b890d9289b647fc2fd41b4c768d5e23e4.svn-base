...
public class MaterialActivitiApplyController {
	...
	public Message deleteById(@PathVariable("id")int id) {
		Message message = new Message();
		message.setSuccess(false);
		int result = materialActivitiApplyService.deleteById(id);
		if (result > 0) {
			message.setSuccess(true);
		}else {
			message.setMsg("删除失败");
	...
	public Message findAccruingAmounts(@RequestParam("did") int did, @RequestParam("list") List<String> gnameList, @RequestParam("totalMoney")String totalMoney) {
		Message message = new Message();
		message.setSuccess(false);
		try {
			Integer count = materialActivitiApplyService.findAccruingAmounts(did, gnameList, totalMoney);
