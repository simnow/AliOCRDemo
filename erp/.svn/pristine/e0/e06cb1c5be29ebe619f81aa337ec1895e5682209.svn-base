...
public class WellPowerServiceImpl implements WellPowerService {
	...
	@Override
	public boolean updateWellPowerByWellIdAndDate(WellPower wellPower) {
		WellPowerExample example=new WellPowerExample();
		Criteria criteria=example.createCriteria();
		criteria.andLogdateEqualTo(wellPower.getLogdate());
		criteria.andWellidEqualTo(wellPower.getWellid());
		if(wellPowerMapper.updateByExampleSelective(wellPower, example)>0){
			return true;
		}
		return false;
	}
	...
	@Override
	public WellPower getAllWellPowerByWellId(int wellId,Date date) {
		WellPowerExample example=new WellPowerExample();
		Criteria criteria=example.createCriteria();
		criteria.andWellidEqualTo(wellId);
		criteria.andLogdateLessThan(date);
