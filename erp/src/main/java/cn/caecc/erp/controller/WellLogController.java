...
//		daily.setSumylj(sumylj);//合计月累计
	...
	public Message updateLogInfo(@RequestBody WellLog welllog){
		...
		if(wellLogService.updateWellLog(welllog)){
			...
			if(doSaveWellDaily(welllog,1,null)){
				//更新动力性能（主要工作、目前进度、钻头外径、泵压、钻速）
				doUpdate(welllog,"wellpower");
				//更新泥浆性能(层位)
				doUpdate(welllog,"wellmud");
				return message;
			}
		}
		message.setSuccess(false);
	...
	private boolean doUpdate(WellLog welllog, String type) {
		//更新动力性能(主要工作、目前进度、钻头外径、泵压、钻速）
		//更新泥浆性能(层位)
		if("wellpower".equals(type)){
			WellPower power=new WellPower();
			power.setWellid(welllog.getWellid());
			power.setLogdate(welllog.getLogdate());
			power.setZygz(welllog.getZygz());
			power.setMqjd(welllog.getMqjd());
			power.setZtwj(welllog.getZtwj());
			power.setPy(welllog.getPy());
			power.setZs(welllog.getZs());
			power.setPl(welllog.getPl());
			wellPowerService.updateWellPowerByWellIdAndDate(power);
		}
		if("wellmud".equals(type)){
			WellMud mud=new WellMud();
			mud.setWellid(welllog.getWellid());
			mud.setLogdate(welllog.getLogdate());
			mud.setJdcw(welllog.getJdcw());
			wellMudService.updateWellPowerByWellIdAndDate(mud);
		}
		return true;
	}
