...
public class WellDxDailyServiceImpl implements WellDxDailyService {
	...
	public Message selectDxWellDaily(Date logDate, Integer wellId) {
		...
		if(null!=list&&list.size()>0){
			Map<String,Object>  wellInfoMap=new HashMap<>();
			for(WellDxDaily well:list){
				WellDxInfo wellDxInfo=null;
				if(null!=wellInfoMap.get(""+well.getWellid())){
					wellDxInfo=(WellDxInfo)wellInfoMap.get(""+well.getWellid());
				}else{
					wellDxInfo=wellDxInfoSerivce.selectDxWellInfoById(well.getWellid());
					wellInfoMap.put(""+well.getWellid(), wellDxInfo);
				}
