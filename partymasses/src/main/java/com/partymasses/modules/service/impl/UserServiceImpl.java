...
public class UserServiceImpl implements UserService{
	...
	public Message updatePassWord(String oldPw,String newPwd){
		...
		//然后添加新手机号，执行更新
		userInfo.setPassword(AesUtil.encrypt(newPwd));
	    ...
	    info.put("roletype", userInfo.getRoletype());
	...
	@Override
	public Message getadminList(int unitId){
		Message message=new Message();
		UserExample example=new UserExample();
		Criteria criteria=example.createCriteria();
	...
	public Message selectUserByMobileOrName(String name, String mobile,String roleType,int unitId,int pageNum,int pageSize) {
		...
		if(Contants.ADMIN.equals(roleType)){
			criteria.andUnitidEqualTo(unitId);
		}
		List<User> result=userMapper.selectByExample(example);
		if(result!=null&&result.size()>0){
			Map<String,Object> countMap=new HashMap<>();
			countMap.put("paginationTotal", 0);
			message.setObj(result);
			message.setAttributes(countMap);
		}
		return message;
	}
